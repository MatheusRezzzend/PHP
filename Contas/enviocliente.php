...
{
    if (!isset($_POST['nome']) && !isset($_POST['cpf']) && !isset($_POST['endereco']) && !isset($_POST['numconta']) && !isset($_POST['agencianum']) && !isset($_POST['saldo'])) {
        return 0;
    } else {
        $cliente = array();
        ...
        $cliente['numconta'] = $_POST['numconta'];
        $cliente['agencianum'] = $_POST['agencianum'];
        $cliente['saldo'] = $_POST['saldo'];

        $agencianum = $cliente['agencianum']; // parte que pega o numero da agencia do cadastro
        $sqlbusca = "SELECT agencianum FROM agencias WHERE agencia LIKE '{$agencianum}'";
        ...
        $cliente['agencianum'] = $conta->agencianum; // sendo assim na posicao conta num agencia existe o valor da agencia cadastrada
    ...
    if (isset($_POST['consulta']) && $_POST['consulta'] != " ") {
        ...
        if ($consulta == 'nome') // seleciona tudo onde a pesquisa for igual a agencia escrita no banco 
        {
            ...
            if (isset($_POST['nome2']) && $_POST['nome2'] != "") {
                ...
                return $secbanco;
            } else {   //reenvia o consulta e faz um breve formulario para enviar o que deseja buscar
                echo "
                    <div class='teste'>
                        <form method='post'>
