...
function salvarcliente($conectar, $cliente)
{
    $salvarcliente = "
        INSERT INTO contas
        (nome, cpf, endereco, numconta, agencianum, saldo)
        VALUES
        (
            '{$cliente['nome']}',
            '{$cliente['cpf']}',
...
function pegaragenciaexclusiva($buscar, $conectar, $tabela)
{
    ...
    $busca2 = "SELECT agencianum FROM agencias WHERE {$tabela} LIKE '{$buscar}'";
    $result2 = mysqli_query($conectar, $busca2);
    $X = 0;

    while ($conta = mysqli_fetch_object($result2)) {
        $agencia = $conta->agencianum; // referencia agencianum dentro do vetor conta
        foreach ($numcontas as $conta) {
            if ($conta['agencianum'] == $agencia) { // verifica se na matriz num contas existe o numero da agencia do cliente 
                $existe[] = $conta;
                $X++;
            }
        }
